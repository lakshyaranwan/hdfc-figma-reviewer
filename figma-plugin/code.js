...
function extractNodeData(node, depth = 0) {
    ...
    if (depth < 4) {
        ...
        if ('effects' in node) {
            baseData.effects = node.effects.map(effect => ({
                type: effect.type,
    ...
    if (node.type === 'TEXT') {
        ...
        if (textNode.fills !== figma.mixed && textNode.fills.length > 0) {
            baseData.fills = textNode.fills.map(fill => {
                const r = 'color' in fill && fill.color ? Math.round(fill.color.r * 255) : 0;
                const g = 'color' in fill && fill.color ? Math.round(fill.color.g * 255) : 0;
                const b = 'color' in fill && fill.color ? Math.round(fill.color.b * 255) : 0;
        ...
        if (textNode.textStyleId && textNode.textStyleId !== figma.mixed) {
            baseData.textStyleId = textNode.textStyleId;
            baseData.textStyleName = ((_a = figma.getStyleById(textNode.textStyleId)) === null || _a === void 0 ? void 0 : _a.name) || null;
        }
        ...
        if ('fillStyleId' in textNode && textNode.fillStyleId && textNode.fillStyleId !== figma.mixed) {
            baseData.fillStyleId = textNode.fillStyleId;
            baseData.fillStyleName = ((_b = figma.getStyleById(textNode.fillStyleId)) === null || _b === void 0 ? void 0 : _b.name) || null;
        }
    }
    // Capture fill/stroke style IDs for non-text nodes too
    if (node.type !== 'TEXT') {
        if ('fillStyleId' in node && node.fillStyleId && node.fillStyleId !== figma.mixed) {
            baseData.fillStyleId = node.fillStyleId;
            baseData.fillStyleName = ((_c = figma.getStyleById(node.fillStyleId)) === null || _c === void 0 ? void 0 : _c.name) || null;
        }
        if ('strokeStyleId' in node && node.strokeStyleId) {
            baseData.strokeStyleId = node.strokeStyleId;
            baseData.strokeStyleName = ((_d = figma.getStyleById(node.strokeStyleId)) === null || _d === void 0 ? void 0 : _d.name) || null;
        }
    }
...
function findNodeByName(name) {
    ...
    if (cachedIds && cachedIds.length > 0) {
        ...
        for (const id of cachedIds) {
            ...
            if (node && isDescendantOfRoot(node))
                return node;
        }
...
function parseDesignValues(suggestion) {
    const result = {};
    ...
    if (radiusMatch)
        result.cornerRadius = parseInt(radiusMatch[1]);
    // Parse opacity
    const opacityPercentMatch = suggestion.match(/opacity[:\s]+(\d+)\s*%/i);
    const opacityDecimalMatch = suggestion.match(/opacity[:\s]+(0\.\d+)/i);
    if (opacityPercentMatch)
        result.opacity = parseInt(opacityPercentMatch[1]) / 100;
    else if (opacityDecimalMatch)
        result.opacity = parseFloat(opacityDecimalMatch[1]);
    // Parse font size
...
function extractNewTextFromSuggestion(suggestion, title) {
...
function extractColorFromSuggestion(suggestion) {
    const lowerSuggestion = suggestion.toLowerCase();
    // Hex color
...
async function applySuggestionToNode(nodeId, location, suggestion, title) {
    ...
    try {
        ...
        if (values.padding && 'paddingLeft' in targetNode) {
            ...
            if (values.padding.all !== undefined) {
                ...
                if (values.padding.top !== undefined)
                    frameNode.paddingTop = values.padding.top;
                if (values.padding.right !== undefined)
                    frameNode.paddingRight = values.padding.right;
                if (values.padding.bottom !== undefined)
                    frameNode.paddingBottom = values.padding.bottom;
                if (values.padding.left !== undefined)
                    frameNode.paddingLeft = values.padding.left;
        ...
        if (values.cornerRadius !== undefined && 'cornerRadius' in targetNode) {
            targetNode.cornerRadius = values.cornerRadius;
        ...
        figma.notify('❌ Error applying suggestion: ' + error.message);
        return { success: false, applied: [] };
    }
}
...
function hasOpaqueSolidFill(node) {
    var _a, _b;
    if (!('fills' in node))
        return false;
    const fills = node.fills;
    ...
    for (const fill of fills) {
        ...
        if (fill.type === 'SOLID') {
            ...
            const nodeOpacity = ('opacity' in node) ? (_b = node.opacity) !== null && _b !== void 0 ? _b : 1 : 1;
            // Consider opaque if combined opacity >= 95%
            if (fillOpacity * nodeOpacity >= 0.95)
                return true;
        }
    }
    return false;
...
function getGradientBackgroundParent(node) {
    let current = node.parent;
    while (current && current.type !== 'PAGE' && current.type !== 'DOCUMENT') {
        const sceneNode = current;
        if (hasGradientFill(sceneNode))
...
async function runTextContrastAudit(nodes) {
    const issues = [];
    ...
    for (const textNode of textNodes) {
        ...
        if (!bgColor)
            continue; // image fill or undetermined — skip
...
let _dsIconNames = new Set();
async function refreshDSIconNames() {
    try {
        const raw = await figma.clientStorage.getAsync('ds_cache');
        if (!raw)
            return;
        const ds = JSON.parse(raw);
        const names = (ds.summary && ds.summary.iconNames) ? ds.summary.iconNames : [];
        _dsIconNames = new Set(names);
    }
    catch (_e) { }
}
// Returns a trimmed DS context object safe to attach to AI messages (no credentials)
async function getDSContext() {
    try {
        const raw = await figma.clientStorage.getAsync('ds_cache');
        if (!raw)
            return null;
...
function runIconContrastAudit(nodes) {
    const issues = [];
    ...
    function getIconFillColor(node) {
        ...
        function walkForFill(n, depth) {
            ...
            if ('children' in n) {
                for (const child of n.children) {
                    const found = walkForFill(child, depth + 1);
                    if (found)
                        return found;
                }
            }
        ...
        if ('children' in node) {
            for (const child of node.children) {
                const found = walkForFill(child, 0);
                if (found)
                    return found;
            }
        }
    ...
    function checkIconContrast(node) {
    ...
    function auditNode(node) {
        ...
        if (isIconInstance(node)) {
            const result = checkIconContrast(node);
            if (result)
                issues.push(result);
...
// ============================================================
async function fetchAndCacheDS(fileKey, pat) {
    try {
        figma.notify('🔄 Loading design system from Figma API…');
        const response = await fetch(`https://api.figma.com/v1/files/${fileKey}?depth=2`, { headers: { 'X-Figma-Token': pat } });
        if (!response.ok) {
...
function extractDSData(figmaFile, fileKey) {
    ...
    for (let i = 0; i < allComponents.length; i++) {
        ...
        if (comp.name.toLowerCase().indexOf('icon') !== -1 ||
    ...
    const colorNames = [];
    for (let i = 0; i < Math.min(paintStyles.length, 80); i++)
        colorNames.push(paintStyles[i].name);
    const textStyleNames = [];
    for (let i = 0; i < Math.min(textStyles.length, 40); i++)
        textStyleNames.push(textStyles[i].name);
    const effectStyleNames = [];
    for (let i = 0; i < Math.min(effectStyles.length, 20); i++)
        effectStyleNames.push(effectStyles[i].name);
    const componentNames = [];
    for (let i = 0; i < Math.min(allComponents.length, 150); i++)
        componentNames.push(allComponents[i].name);
    const iconNames = [];
    for (let i = 0; i < Math.min(iconComponents.length, 100); i++)
        iconNames.push(iconComponents[i].name);
    ...
    function extractHexFromNode(node) {
        const fills = node.fills;
    ...
    function walkForColorValues(node) {
        if (!node)
            return;
        if (node.styles && node.styles.fill) {
            ...
            if (!seenStyleIds.has(styleId)) {
                ...
                if (styleEntry) {
                    ...
                    if (hex)
                        colorTokenMap.push({ name: styleEntry.name, hex });
                }
            }
        }
        const children = node.children;
...
async function loadDSFromCurrentFile() {
    try {
...
(async () => {
    ...
    const cacheRaw = await figma.clientStorage.getAsync('ds_cache');
    const cacheTimestampRaw = await figma.clientStorage.getAsync('ds_cache_timestamp');
    const fileKey = await figma.clientStorage.getAsync('ds_file_key');
...
figma.ui.onmessage = async (msg) => {
    ...
    if (msg.type === 'analyze') {
        ...
        if (selectionData.nodes)
            cacheNodeNames(selectionData.nodes);
        _analysisRootIds = figma.currentPage.selection.map(n => n.id);
    ...
    if (msg.type === 'apply-suggestion') {
        const result = await applySuggestionToNode(msg.nodeId, msg.location, msg.suggestion, msg.title);
        figma.ui.postMessage({
            type: 'apply-complete',
                ...
                for (const [name, ids] of _nodeNameCache.entries()) {
                    ...
                    if (nameLower === searchLower || nameLower.includes(searchLower) || searchLower.includes(nameLower)) {
                        for (const id of ids) {
                            ...
                            if (found && isDescendantOfRoot(found)) {
                                targetNode = found;
                                break;
                            }
                            if (found && !fallback)
    ...
    if (msg.type === 'create-annotation') {
        ...
        try {
            ...
            const entries = [];
            if (interaction)
                entries.push({ type: 'interaction', value: interaction });
            if (role)
                entries.push({ type: 'role', value: role });
            if (label)
                entries.push({ type: 'label', value: label });
