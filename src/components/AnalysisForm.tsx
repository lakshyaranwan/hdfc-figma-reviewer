...
export const AnalysisForm = ({
  ...
  const handleAnalyze = async () => {
    ...
    if (promptMode === "simple") {
      const categoryLabels = selectedCategories.map(id => ANALYSIS_CATEGORIES.find(cat => cat.id === id)?.label).join(", ");
  ...
  return <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        <div className="space-y-2">
          ...
          <Input id="figma-url" type="url" placeholder="https://www.figma.com/design/..." value={figmaUrl} onChange={e => setFigmaUrl(e.target.value)} disabled={isAnalyzing} className="bg-background border-border focus:ring-primary" />
          ...
          <Tabs value={promptMode} onValueChange={v => setPromptMode(v as "simple" | "manual")}>
            ...
            <TabsContent value="simple" className="space-y-3 mt-3">
              ...
              <div className="space-y-2.5">
                {ANALYSIS_CATEGORIES.map(category => <div key={category.id} className="flex items-center space-x-2">
                    <Checkbox id={category.id} checked={selectedCategories.includes(category.id)} onCheckedChange={() => toggleCategory(category.id)} disabled={isAnalyzing} />
                    <Label htmlFor={category.id} className="text-sm font-normal cursor-pointer leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70">
                      {category.label}
                    </Label>
                  </div>)}
              </div>
            ...
            <TabsContent value="manual" className="space-y-3 mt-3">
              ...
              <Textarea placeholder="Example: Focus on mobile responsiveness and button states. Check if all CTAs are clearly visible..." value={customPrompt} onChange={e => setCustomPrompt(e.target.value)} disabled={isAnalyzing} className="min-h-[120px] bg-background border-border focus:ring-primary" />
              ...
              <div className="flex items-center justify-between pt-4 border-t">
                ...
                <Switch id="suggestions-manual" checked={includeSuggestions} onCheckedChange={setIncludeSuggestions} disabled={isAnalyzing} />
              </div>
            </TabsContent>
          </Tabs>
        </div>

        <Button onClick={handleAnalyze} disabled={isAnalyzing} className="w-full bg-gradient-to-r from-primary to-primary/90 hover:opacity-90 transition-opacity">
          {isAnalyzing ? <>
